# PEfile_Path = "D:\ttn\逆向分析\实验二\tlntsess.exe"
# -*-coding:utf-8-*-
...
#提取ASCII字符串
    ...
    def is_valid_pe(self):
        temp_ptr = self.get_dword(self.data[0x3c:0x40])
    ...
    def rva_to_offset(self,rva):
        h32_size_ptr = self.get_dword(self.data[0x3c:0x40]) + 0x14
        h32_size = self.get_dword(self.data[h32_size_ptr:h32_size_ptr+2] + b'\x00\x00')
        temp_rva = self.get_dword(self.data[0x3c:0x40]) + 0x18 + h32_size
        while True:
    ...
    def parse_import_table(self):
        ...
        self.import_table_rva = self.get_dword(self.data[self.pe_rva:self.pe_rva+4])
        self.import_table_size = self.get_dword(self.data[self.pe_rva+4:self.pe_rva+8])
        self.import_table_offset = self.get_dword(self.data[self.pe_rva+8:self.pe_rva+12])
        ...
        while True:
            ...
            iid_list_temp.append(self.get_string(self.rva_to_offset(self.get_dword(self.data[ptr_temp+12:ptr_temp+16]))))
